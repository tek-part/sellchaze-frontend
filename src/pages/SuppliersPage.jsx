...
export default function SuppliersPage() {
    ...
    const seesAdminSupplierDirectory = isAdmin || roles.includes('Staff') || roles.includes('Manager');
    ...
    useEffect(() => {
        setLoading(true);
        setErr('');
        api.get('/suppliers', { params: loadParams() })
            .then(({ data }) => {
                setRows(data.data ?? []);
    ...
    const isMerchantView = !seesAdminSupplierDirectory && roles.includes('Merchant');
    const showAdminActions = isAdmin;

    const cols = isMerchantView
        ? [
              { key: 'name', header: t('col_name') },
              { key: 'email', header: t('email') },
              { key: 'partnership_status', header: t('col_partnership') },
          ]
        : [
              { key: 'name', header: t('col_name') },
              { key: 'email', header: t('email') },
    ...
    const handleExportCurrent = () => {
        exportRowsToExcel(cols, rows.map(rowToExport), `suppliers-p${page}`, { sheetName: 'Suppliers' });
    };
    ...
    const handleExportAll = async () => {
        ...
        try {
            ...
            const all = await fetchAllPages('/suppliers', { ...base, per_page: 100 });
            exportRowsToExcel(cols, all.map(rowToExport), 'suppliers-all', { sheetName: 'Suppliers' });
        } catch (e) {
    ...
    const runDelete = async () => {
        ...
        try {
            ...
            toast.success(t('supplier_deleted_toast'));
    ...
    const runBulkDelete = async () => {
        ...
        setDeleteLoading(true);
        try {
            await api.post('/suppliers/bulk-destroy', { ids });
            toast.success(t('suppliers_bulk_deleted_toast', { count: ids.length }));
    ...
    const runToggle = async () => {
        ...
        try {
            ...
            await api.patch(`/suppliers/${toggleTarget.id}/status`, { is_active: next });
    ...
    const emptyColSpan = isMerchantView ? 4 : showAdminActions ? 6 : 4;

    return (
        <div className="space-y-5">
            <div className="border-s-4 border-brand ps-4">
                <h1 className="text-2xl font-semibold tracking-tight text-slate-900">{t('suppliers')}</h1>
                    ...
                    <table className="min-w-full text-start text-sm">
                        <thead className="bg-surface-muted/90 text-xs font-semibold uppercase tracking-wide text-slate-600">
                            <tr>
                                ...
                                {isMerchantView ? (
                                    <th className="px-4 py-3.5">{t('col_partnership')}</th>
                                ) : (
                                    <>
                        ...
                        <tbody>
                            ...
                            {rows.map((row) => (
                                ...
                                >
                                    ...
                                    {isMerchantView ? (
                                        <td className="px-4 py-3 text-slate-700">{row.partnership_status ?? '—'}</td>
                                    ) : (
                                        <>
            ...
            <ConfirmDialog
                ...
                title={t('confirm_delete_supplier_title')}
                body={t('confirm_delete_supplier_body', { name: confirmDelete?.name ?? '' })}
            ...
            <ConfirmDialog
                ...
                body={t('confirm_bulk_delete_suppliers_body')}
                confirmLabel={t('action_delete')}
                cancelLabel={t('cancel')}
                danger
                loading={deleteLoading}
