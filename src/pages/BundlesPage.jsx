...
export default function BundlesPage() {
    ...
    useEffect(() => {
        if (!permissions.includes('bundles-list')) {
            return;
        }
        setLoading(true);
        setErr('');
        api.get('/bundles', { params: loadParams() })
            .then(({ data }) => {
                setRows(data.data ?? []);
    ...
    const handleExportAll = async () => {
        ...
        try {
            ...
            const all = await fetchAllPages('/bundles', { ...base, per_page: 100 });
            exportRowsToExcel(
                exportColumns,
                all.map((r) => ({
                    ...r,
    ...
    const runDeleteOne = async () => {
        ...
        setDeleting(true);
        try {
            await api.delete(`/bundles/${confirmOne}`);
    ...
    const runBulkDelete = async () => {
        ...
        setDeleting(true);
        try {
            const { data } = await api.post('/bundles/bulk-destroy', { ids });
            toast.success(
                    ...
                    <table className="min-w-full text-start text-sm">
                        ...
                        <tbody>
                            {rows.length === 0 && !loading ? (
                                <tr>
                                    <td
                                        colSpan={canBulkDeleteByActivity && can('bundles-delete') ? 6 : 5}
