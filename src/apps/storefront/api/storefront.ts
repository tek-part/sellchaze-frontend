...
import type { ApiCategory, ApiPaginated, ApiProduct, ApiReview, ApiStoreSummary } from './types';
...
export function getProducts(params: { category?: string; perPage?: number; page?: number } = {}): Promise<ApiPaginated<ApiProduct>> {
...
/* ---- auth / account ---- */
export const login = (body: { email: string; password: string }): Promise<{ data: unknown }> => apiSend('/auth/login', 'POST', body);
export const register = (body: Record<string, unknown>): Promise<{ data: unknown }> => apiSend('/auth/register', 'POST', body);
