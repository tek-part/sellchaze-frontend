/**
 * Storefront foundation component library — public surface.
 *
 * The reusable, token-driven `.sf-*` primitives the shared route pages (auth, account, cart,
 * checkout, wishlist, static, 404, error boundary), the library PDP and the themes' chrome compose
 * from. Every component is skinned by `../base.css` (loaded app-level by `main.tsx`) and reads only
 * `var(--token)`, so a scheme/settings change restyles them with no re-render and each theme's
 * tokens do the visual differentiation. Grouped by the design-bible taxonomy (Actions, Layout,
 * Typography, Commerce, Feedback, Forms, Cards, Overlays, Navigation, Media).
 */
