/**
 * Typed, fail-safe readers over a section instance's `ThemeSettings` (an untyped
 * Record<string, ThemeSettingValue>). Every getter coerces and falls back, so a missing or
 * malformed setting never throws — sections always render. Mirrors the documented field types
 * (text/textarea/richtext/url/image → string, toggle → boolean, number/range → number, select → string).
 */
...
/**
 * Parse a "richtext"/textarea setting that encodes a simple list — one item per line, or "term |
 * definition" for FAQ-style pairs. Blank lines are dropped. This is how the doc models list settings
 * where the page-builder has no repeater field type.
 */
