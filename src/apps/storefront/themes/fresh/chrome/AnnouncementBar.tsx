/**
 * Fresh AnnouncementBar — slim lime top band from the `announcement_text` setting (or the store's
...
export function AnnouncementBar(props: AnnouncementBarProps): ReactElement | null {
  ...
  return (
    <div className="fr-announcement" role="region" aria-label="Announcement">
      <div className="lib-container fr-announcement__inner">
        <span aria-live="polite" className="fr-announcement__text">
          {url ? <a href={url}>{message}</a> : message}
        </span>
        <button type="button" className="fr-announcement__close" aria-label={t('common.close')} onClick={() => setDismissed(true)}>
