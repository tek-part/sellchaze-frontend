/**
 * Naseem DefaultLayout — global chrome shell: AnnouncementBar → Header → <main> → Footer, plus the
 * CartDrawer, MobileNav and SearchOverlay it controls. Loads the merchant's Google fonts, and
 * projects the card-style settings as data attributes so `theme.css` can restyle library cards.
 */
...
const GOOGLE_FAMILIES = new Set(['Tajawal', 'Cairo', 'IBM Plex Sans Arabic', 'Almarai', 'Rubik', 'Inter', 'Poppins']);
...
function useGoogleFonts(families: ReadonlyArray<unknown>): void {
  ...
  useEffect(() => {
    ...
    const id = 'nsm-google-fonts';
    const query = key.split('|').map((f) => `family=${encodeURIComponent(f).replace(/%20/g, '+')}:wght@400;500;700`).join('&');
...
export function DefaultLayout(props: LayoutRenderProps): ReactElement {
  ...
  const cardStyle = typeof settings['card_style'] === 'string' ? settings['card_style'] : 'card';
  const cardRatio = typeof settings['card_ratio'] === 'string' ? settings['card_ratio'] : 'square';
      ...
      <CartProvider>
        ...
        >
          ...
          <Header
            ...
            nav={nav}
            style={typeof settings['header_style'] === 'string' ? settings['header_style'] : 'classic'}
          ...
          <main id="sf-main" className="nsm-main">{children}</main>
