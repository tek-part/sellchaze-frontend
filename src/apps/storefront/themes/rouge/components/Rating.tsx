/** Rouge Rating — gilt stars + humanist count; self-hides with no data. */
...
export function Rating(props: RatingProps): ReactElement | null {
  ...
  return (
    <span className={cn('rge-rating', className)} role="img" aria-label={t('product.ratedOfMax', { value: value.toFixed(1), max })}>
      <span className="rge-rating__stars" aria-hidden>
        {Array.from({ length: max }, (_, i) => (
          <IconStar key={i} width={15} height={15} style={{ opacity: i < rounded ? 1 : 0.25 }} />
        ))}
      </span>
      {showCount && typeof count === 'number' ? <span className="rge-rating__count rge-num">({count})</span> : null}
    </span>
  );
}
