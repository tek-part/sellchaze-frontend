...
export function QuantityStepper(props: QuantityStepperProps): ReactElement {
  const { value, onChange, min = 1, max = 99, label = 'Quantity', className } = props;
  ...
  return (
    <div className={cn('hh-qty', className)} role="group" aria-label={label}>
      <button type="button" className="hh-qty__btn" onClick={dec} disabled={value <= min} aria-label="Decrease quantity">
        −
      </button>
      <span className="hh-qty__value" aria-live="polite">
        {value}
      </span>
      <button type="button" className="hh-qty__btn" onClick={inc} disabled={value >= max} aria-label="Increase quantity">
        +
      </button>
    </div>
  );
