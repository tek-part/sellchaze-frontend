...
export function QuantityStepper(props: QuantityStepperProps): ReactElement {
  ...
  return (
    <div className={cn('vlt-qty', className)}>
      <button type="button" className="vlt-qty__btn" aria-label={t('product.decreaseQuantity')} aria-controls={id} disabled={disabled || value <= min} onClick={() => set(value - 1)}>−</button>
      ...
      <button type="button" className="vlt-qty__btn" aria-label={t('product.increaseQuantity')} aria-controls={id} disabled={disabled || value >= max} onClick={() => set(value + 1)}>+</button>
    </div>
  );
}
