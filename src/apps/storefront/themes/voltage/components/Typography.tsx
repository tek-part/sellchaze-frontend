/**
 * Voltage typography — Eyebrow (uppercase mono, lime), Heading (grotesque), Text (grotesque body).
 * Size maps to a `--fs-*` token; the semantic tag is chosen via `as` (visual scale ≠ outline).
 */
...
export interface EyebrowProps extends HTMLAttributes<HTMLElement> { as?: ElementType; children: ReactNode; }
export function Eyebrow(props: EyebrowProps): ReactElement {
  const { as: Tag = 'span', className, children, ...rest } = props;
  return <Tag className={cn('vlt-eyebrow', className)} {...rest}>{children}</Tag>;
}
...
export function Heading(props: HeadingProps): ReactElement {
  ...
  return <Tag className={cn('vlt-heading', className)} style={sizeStyle(size, style)} {...rest}>{children}</Tag>;
}
...
export function Text(props: TextProps): ReactElement {
  ...
  return (
    <Tag className={cn('vlt-text', muted && 'vlt-text--muted', className)} style={sizeStyle(size, style)} {...rest}>
      {children}
    </Tag>
  );
}
