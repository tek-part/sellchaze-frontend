...
export function OrdersSection(_props: SectionRenderProps): ReactElement {
  ...
  if (ordersQ.loading) return <div className="vlt-account-loading"><Spinner label="Loading orders" /></div>;
  if (orders.length === 0) {
    return (
      <div className="vlt-empty vlt-empty--pad">
        <span className="vlt-empty__title">No orders yet</span>
        <span className="vlt-empty__text">When you place an order, it will appear here.</span>
      </div>
    );
  }
        ...
        <li key={order.id}>
          <a href={`/account/orders/${order.number}`} className="vlt-log__row">
            <span className="vlt-log__main">
              <span className="vlt-log__title">Order {order.number}</span>
