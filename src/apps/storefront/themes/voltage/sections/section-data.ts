/**
 * Voltage — fail-safe readers over StorefrontContext.data (the shared storefront data contract the
 * pages provide). Voltage's own copy; reads shared catalog view-models. Empty on absent/malformed
 * data so sections show empty/skeleton states rather than crashing.
 */
...
interface Data {
  ...
  pageHeader?: { title?: string; description?: string; image?: string; breadcrumbs?: ReadonlyArray<Breadcrumb> };
}
...
export function breadcrumbsOf(context: StorefrontContext): ReadonlyArray<Breadcrumb> {
  return asArray(bag(context).pageHeader?.breadcrumbs);
}
