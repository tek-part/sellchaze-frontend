/**
 * product-tabs (Techno) — the library tablist with the theme card (spec chips + compare).
 */
import { useState, type KeyboardEvent, type ReactElement } from 'react';
import type { SectionRenderProps } from '../../../theme-engine/rendering';
import { cn } from '../../../../../shared/utils/cn';
import { productTabsSchema } from '../../../sections-lib/components/ProductTabs';
import { LibCarousel, LibEmpty, LibGrid, LibSection, bool, gridStyle, list, num, spacingStyle, str, useLibId, useLibT, useSectionData, useSectionSettings } from '../../../sections-lib';
import { TkProductCard } from '../components/ProductCard';

export function TkProductTabs(props: SectionRenderProps): ReactElement | null {
  ...
  const tablist = (
    <div className={cn('lib-tabs', 'tk-tabs', `lib-tabs--${str(s, 'align', 'center')}`)} role="tablist" aria-label={t('tabs')} onKeyDown={onKey}>
      {tabs.map((tab, i) => (
        <button
          key={i}
          type="button"
  ...
  const cards = products.map((p, i) => <TkProductCard key={p.id} product={p} {...card} eager={i < columns} />);
